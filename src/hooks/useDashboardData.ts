...
export interface DashboardMetrics {
  store: {
    total: {
      ...
      topProductsSold: Array<{       // Top 10 produtos mais vendidos
        productId: string;
        productName: string;
        saleQuantity: number;
        percentage: number;
      }>;
      topStreamersSales: Array<{     // Top 10 criadores que mais venderam
        streamerId: string;
        streamerName: string;
        totalSales: number;
        totalRevenue: number;
    ...
    last30Days: {
      ...
      topProductsSold: Array<{       // Top 10 produtos mais vendidos últimos 30 dias
        productId: string;
        productName: string;
        saleQuantity: number;
        percentage: number;
...
export const useDashboardData = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const newUsers30d = usersData.data.filter((user: any) => 
        new Date(user.createdAt) > now30DaysAgo
      ).length;

      // Calcular usuários por status (true/false para active e blocked)
      const usersActive = usersData.data.filter((user: any) => user.active === true).length;
      const usersInactive = usersData.data.filter((user: any) => user.active === false || !user.active).length;
      const usersBlocked = usersData.data.filter((user: any) => user.blocked === true).length;
      const usersUnblocked = usersData.data.filter((user: any) => user.blocked === false || !user.blocked).length;

      // Calcular usuários dos últimos 30 dias por status
      const users30d = usersData.data.filter((user: any) => 
        new Date(user.createdAt) > now30DaysAgo
      );
      const usersActive30d = users30d.filter((user: any) => user.active === true).length;
      const usersInactive30d = users30d.filter((user: any) => user.active === false || !user.active).length;
      const usersBlocked30d = users30d.filter((user: any) => user.blocked === true).length;
      const usersUnblocked30d = users30d.filter((user: any) => user.blocked === false || !user.blocked).length;
      ...
      const streamersPublic = streamersData.data.filter((streamer: any) => streamer.public === true).length;
      const streamersPrivate = streamersData.data.filter((streamer: any) => streamer.public === false || !streamer.public).length;
      
      const newStreamers30d = streamersData.data.filter((streamer: any) => 
        new Date(streamer.createdAt) > now30DaysAgo
      ).length;

      // Processar solicitações de streamers (usando dados reais)
      const requestsPending = streamerRequestsData.data.filter((req: any) => req.status === 'PENDING').length;
      const requestsApproved = streamerRequestsData.data.filter((req: any) => req.status === 'APPROVED').length;
      const requestsRejected = streamerRequestsData.data.filter((req: any) => req.status === 'REJECTED').length;

      // Solicitações dos últimos 30 dias
      const requests30d = streamerRequestsData.data.filter((req: any) => 
        new Date(req.createdAt) > now30DaysAgo
      );
      const requestsPending30d = requests30d.filter((req: any) => req.status === 'PENDING').length;
      const requestsApproved30d = requests30d.filter((req: any) => req.status === 'APPROVED').length;
      const requestsRejected30d = requests30d.filter((req: any) => req.status === 'REJECTED').length;

      // === CALCULAR SOLICITAÇÕES PENDENTES ===
      
      // Withdrawal Requests pendentes
      const withdrawalRequestsPending = withdrawalsData.data.filter((req: any) => 
      ...
      const productStreamerPending = productStreamersData.data.filter((ps: any) => 
        ps.status === 'PENDING' || ps.status === 'pending'
      ).length;

      // Order-Cancel pendentes
      const orderCancelPending = orderCancelData.data.filter((oc: any) => 
        oc.status === 'PENDING' || oc.status === 'pending'
      ).length;

      // === PROCESSAR STATUS DOS PRODUTOS ===
      
      // Debug: verificar dados dos produtos
      console.log('ProductStreamers Total:', productStreamersData.data.length);
      console.log('ProductStreamers Sample:', productStreamersData.data.slice(0, 5));
      
      // Verificar todos os status únicos que existem
      const uniqueStatuses = [...new Set(productStreamersData.data.map((p: any) => p.status))];
      console.log('Unique Statuses Found:', uniqueStatuses);
      
      // Calcular status dos produtos baseado em product-streamer
      const productsByStatus = productStreamersData.data.reduce((acc: any, product: any) => {
        const status = product.status || 'NO_STATUS';
        acc[status] = (acc[status] || 0) + 1;
        return acc;
      }, {});

      console.log('Products By Status Count:', productsByStatus);
      ...
      const productsInactive = 
        ...
        productsByStatus['disabled'] ||
        productsByStatus['NO_STATUS'] || 0;

      // === PROCESSAR PRODUTOS BASE (/products) ===
      
      console.log('Base Products Total:', productsData.data.length);
      console.log('Base Products Sample:', productsData.data.slice(0, 3));
      
      // Calcular status dos produtos base
      const baseProductsByStatus = productsData.data.reduce((acc: any, product: any) => {
      ...
      console.log('Base Products By Status:', baseProductsByStatus);
      ...
      console.log(`Filtragem de pedidos: ${paidOrders.length} válidos de ${ordersData.data.length} totais`);
      
      // Métricas totais da loja (apenas pedidos pagos/concluídos)
      const totalShippingRevenue = paidOrders.reduce(
        (sum: number, order: any) => sum + (order.totalAmountShipping || 0), 0
      );
      ...
      const totalCommissionsPaid = commissionsData.data.reduce(
        (sum: number, commission: any) => sum + (commission.commissionValue || 0), 0
      );
      
      // Calcular quantidade total de produtos vendidos
      const totalProductsSold = commissionsData.data.reduce(
        (sum: number, commission: any) => sum + (commission.productSaleQuantity || 0), 0
      );
      ...
      const totalAverageTicket = totalOrdersCount > 0 ? 
        paidOrders.reduce((sum: number, order: any) => sum + (order.totalAmount || 0), 0) / totalOrdersCount : 0;

      // Métricas dos últimos 30 dias da loja (apenas status válidos)
      const ordersLast30Days = paidOrders.filter((order: any) => 
        new Date(order.createdAt) > now30DaysAgo
      );
      ...
      const commissions30d = commissionsData.data.filter((commission: any) => 
        new Date(commission.createdAt) > now30DaysAgo
      );
      ...
      const averageTicket30d = orders30dCount > 0 ? 
        ordersLast30Days.reduce((sum: number, order: any) => sum + (order.totalAmount || 0), 0) / orders30dCount : 0;

      // === PROCESSAR DADOS DE DOAÇÕES ===

      // Doações totais
      const allDonationsAmount = donationsData.data.reduce(
        (sum: number, donation: any) => sum + (donation.amount || 0), 0
      );
      const allDonationsCount = donationsData.data.length;
      const allDonationsAverage = allDonationsCount > 0 ? allDonationsAmount / allDonationsCount : 0;
      const allDonationsHighest = Math.max(...donationsData.data.map((d: any) => d.amount || 0), 0);

      // Doações dos últimos 30 dias
      const donationsLast30Days = donationsData.data.filter((donation: any) => 
        new Date(donation.createdAt) > now30DaysAgo
      );
      ...
      const donations30dAverage = donations30dCount > 0 ? donations30dAmount / donations30dCount : 0;
      const donations30dHighest = Math.max(...donationsLast30Days.map((d: any) => d.amount || 0), 0);

      // Top streamers por doações (30 dias)
      const streamerDonations30d = donationsLast30Days.reduce((acc: any, donation: any) => {
      ...
      const topStreamers30d = Object.values(streamerDonations30d)
        ...
        .slice(0, 10)
        .map((streamer: any) => {
          const streamerData = streamersData.data.find((s: any) => s.id === streamer.streamerId);
          return {
      ...
      const streamerDonationsTotal = donationsData.data.reduce((acc: any, donation: any) => {
        const streamerId = donation.streamerId;
        if (!acc[streamerId]) {
          acc[streamerId] = { streamerId, totalAmount: 0 };
        }
      ...
      const orders30DaysAll = ordersData.data.filter((order: any) => 
        new Date(order.createdAt) > now30DaysAgo
      );
      
      const ordersByStatus30d = orders30DaysAll.reduce((acc: any, order: any) => {
        const status = order.currentStatus || 'UNKNOWN';
        acc[status] = (acc[status] || 0) + 1;
        return acc;
      }, {});
      ...
      const calculateTopProductsUsage = (productStreamerData: any[]) => {
        ...
        const totalUsages = sortedProducts.reduce((sum, product) => sum + product.usageCount, 0);

        return sortedProducts.map(product => ({
          ...product,
          percentage: totalUsages > 0 ? (product.usageCount / totalUsages) * 100 : 0
        }));
      };
      ...
      const calculateTopProductsSold = (commissionsData: any[]) => {
        ...
        commissionsData.forEach((commission: any) => {
          if (commission.productStreamer && commission.productSaleQuantity) {
            ...
            } else if (commission.productStreamer && typeof commission.productStreamer === 'object') {
              // If it's an object, try to get name or id
              productName = commission.productStreamer.name || commission.productStreamer.id || 'Produto Desconhecido';
            } else {
        ...
        // Calculate percentages
        const totalSales = sortedProducts.reduce((sum, product) => sum + product.saleQuantity, 0);

        return sortedProducts.map(product => ({
          ...product,
          percentage: totalSales > 0 ? (product.saleQuantity / totalSales) * 100 : 0
        }));
      };
      ...
      const calculateTopCreators = (data: any[], streamersData: any) => {
        ...
        const streamerProductCount = new Map<string, { streamerId: string; streamerName: string; productCount: number; }>();
        ...
        activeProducts.forEach((item: any) => {
          ...
          if (streamerId) {
            ...
            if (streamerData) {
              ...
              streamerName = streamerData.atname || streamerData.name || streamerId;
            } else if (item.streamer) {
              // Fallback para dados do produto-streamer
              streamerName = item.streamer.atname || item.streamer.name || streamerId;
            }
      ...
      const topProductsUsageTotal = calculateTopProductsUsage(productStreamersData.data || []);
      const topCreatorsTotal = calculateTopCreators(productStreamersData.data || [], streamersData);
      
      // Calculate top products sold from commission data
      const topProductsSoldTotal = calculateTopProductsSold(commissionsData.data || []);
      
      // Debug: let's check what date fields are available
      if (productStreamersData.data && productStreamersData.data.length > 0) {
        console.log('Sample product-streamer data:', productStreamersData.data[0]);
      }
      
      const topProductsUsageLast30Days = calculateTopProductsUsage(
        (productStreamersData.data || []).filter((ps: any) => {
          // Try multiple possible date field names
          const dateField = ps.created_at || ps.createdAt || ps.date || ps.createDate;
      ...
      const topProductsSoldLast30Days = calculateTopProductsSold(commissions30d || []);

      // Calculate top streamers by sales from commission-streamer data
      const calculateTopStreamersSales = (commissionsData: any[], streamersData: any) => {
        const streamerSales = new Map<string, {
          streamerId: string;
          streamerName: string;
          totalSales: number;
          totalRevenue: number;
        ...
        commissionsData.forEach((commission: any) => {
          if (commission.streamerId && commission.productSaleQuantity && commission.commissionValue) {
        ...
        const totalSalesSum = sortedStreamers.reduce((sum, streamer) => sum + streamer.totalSales, 0);

        return sortedStreamers.map(streamer => ({
          ...streamer,
          percentage: totalSalesSum > 0 ? (streamer.totalSales / totalSalesSum) * 100 : 0
        }));
      };

      // Calculate top streamers sales for total and last 30 days
      const topStreamersSalesTotal = calculateTopStreamersSales(commissionsData.data || [], streamersData);
      const topStreamersSalesLast30Days = calculateTopStreamersSales(commissions30d || [], streamersData);
      ...
      console.log('Total wallets:', walletsData.data.length);
      console.log('Primeiras 3 wallets completas:', JSON.stringify(walletsData.data.slice(0, 3), null, 2));
      
      // Verificar todos os tipos existentes
      const allTypes = walletsData.data.map((w: any) => w.type);
      console.log('Todos os tipos:', allTypes);
      ...
      console.log('Store wallets encontradas:', storeWallets.length);
      console.log('Donation wallets encontradas:', donationWallets.length);
      
      if (storeWallets.length > 0) {
        console.log('Exemplo store wallet:', JSON.stringify(storeWallets[0], null, 2));
      }
      if (donationWallets.length > 0) {
        console.log('Exemplo donation wallet:', JSON.stringify(donationWallets[0], null, 2));
      }
      ...
      storeWallets.forEach((wallet: any) => {
        ...
        const pendingBalance = parseFloat(wallet.pendingBalance || wallet.pending_balance) || 0;
        storeBalanceReleased += balance;
        storeBalancePending += pendingBalance;
        console.log(`Store wallet ${wallet.id}: balance=${balance}, pending=${pendingBalance}, type="${wallet.type}"`);
      });
      ...
      donationWallets.forEach((wallet: any) => {
        ...
        console.log(`Donation wallet ${wallet.id}: balance=${balance}, pending=${pendingBalance}, type="${wallet.type}"`);
      });
      
      // Se não encontrarmos wallets separadas por tipo, vamos mostrar todas como "indefinidas"
      if (storeWallets.length === 0 && donationWallets.length === 0 && walletsData.data.length > 0) {
        console.warn('NENHUMA WALLET ENCONTRADA COM TIPOS CONHECIDOS - SOMANDO TODAS');
        walletsData.data.forEach((wallet: any) => {
          ...
          console.log(`Wallet indefinida ${wallet.id}: balance=${balance}, pending=${pendingBalance}, type="${wallet.type}"`);
        });
      }
      
      console.log('=== RESULTADOS WALLETS ===');
      console.log('Store Released:', storeBalanceReleased);
      console.log('Store Pending:', storeBalancePending);
      console.log('Donation Released:', donationBalanceReleased);
      console.log('Donation Pending:', donationBalancePending);

      // === CALCULAR MÉTRICAS DE SAQUES ===
      
      console.log('=== DEBUG WITHDRAWALS ===');
      console.log('Total withdrawals:', withdrawalsData.data.length);
      
      // Filtrar saques completados
      const completedWithdrawals = withdrawalsData.data.filter((withdrawal: any) => 
        withdrawal.status === 'COMPLETED' || withdrawal.status === 'completed'
      );
      
      // Filtrar saques pendentes  
      const pendingWithdrawals = withdrawalsData.data.filter((withdrawal: any) => 
      ...
      const completedTotals = completedWithdrawals.reduce((acc: any, w: any) => ({
      ...
      const pendingTotals = pendingWithdrawals.reduce((acc: any, w: any) => ({
        totalAmount: acc.totalAmount + (parseFloat(w.amount) || 0),
        totalFee: acc.totalFee + (parseFloat(w.fee) || 0),
        finalAmount: acc.finalAmount + (parseFloat(w.finalAmount) || 0),
        count: acc.count + 1
      ...
      const now30DaysAgoWithdrawals = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
      ...
      const completed30DaysTotals = completed30Days.reduce((acc: any, w: any) => ({
        totalAmount: acc.totalAmount + (parseFloat(w.amount) || 0),
        totalFee: acc.totalFee + (parseFloat(w.fee) || 0),
        finalAmount: acc.finalAmount + (parseFloat(w.finalAmount) || 0),
        count: acc.count + 1
      ...
      const pending30DaysTotals = pending30Days.reduce((acc: any, w: any) => ({
        totalAmount: acc.totalAmount + (parseFloat(w.amount) || 0),
        totalFee: acc.totalFee + (parseFloat(w.fee) || 0),
        finalAmount: acc.finalAmount + (parseFloat(w.finalAmount) || 0),
        count: acc.count + 1
      ...
      console.log('=== RESULTADOS WITHDRAWALS ===');
      console.log('Completed Total:', completedTotals);
      console.log('Pending Total:', pendingTotals);
      console.log('Completed 30d:', completed30DaysTotals);
      console.log('Pending 30d:', pending30DaysTotals);
      ...
      const dashboardMetrics: DashboardMetrics = {
        ...
        operational: {
          ...
          pendingWithdrawals: withdrawalsData.data.filter((w: any) => w.status === 'PENDING').length,
